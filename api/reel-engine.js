...
import { getSkeletonReferences } from '../lib/skeleton-references.js';
...
export default async function(req,res){
 ...
 const visualReferenceBank=await getSkeletonReferences();
 let director=null;
 try{
...
Never invent external APIs or video services.`;
 ...
 res.json({ok:true,engine:'HAxBRO Independent AI-Directed 3D Reel System',free:true,externalPipeline:false,duration,aspectRatio:'9:16',character,trend,scriptProvided:Boolean(script),director:{mode:'AI cinematic planning + deterministic local rendering',provider:'HAxBRO managed AI director',continuity:director.continuity||'preserve identity, pose intent and spatial continuity',camera:director.camera||'shot-aware cinematic camera'},animation:{leader:'KAI 6.0',workers:['KAI 3.0','KAI 3.4'],visualAuthority:'KAI 56',mode:dance?'full-body dance':'semantic full-body performance',webgl:true,depthBuffer:true,lights:true,shadows:true,articulatedJoints:true,weightShift:true,temporalContinuity:true},visualTraining:{referenceCount:visualReferenceBank.length,requestedReferenceCount:170,referenceType:'live Wikimedia Commons skeleton anatomy references',purpose:'character anatomy, poses, camera views, lighting, depth, clothing and motion consistency'},visualReferenceBank:{count:visualReferenceBank.length,sharedWith:['KAI 6.0 animator director','KAI 3.0','KAI 3.4','KAI 56 visual inspector'],references:visualReferenceBank},render:{background:'3D cyber stage',motion:director.motion||'shot-aware full-body motion with weight shift and articulated limbs',character:character.label,captions:true,particles:true,musicTrack:'native',exactElements:'renderer-controlled'},scenes,availableCharacters:CHARACTERS,availableTrends:TRENDS});
}