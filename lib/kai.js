...
// Provider order: Google first, OpenAI second, then optional configured providers.
// The final emergency fallback remains KAI-51 web research (and can be backed by Cerebrium externally).
const KAI_MODELS = [
...
export async function complete({ system = '', prompt, maxTokens = 1800, username = '', history = '' } = {}) {
  ...
  try {
    throw new Error('All configured language-model providers failed');
  } catch (aiError) {
