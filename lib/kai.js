...
const KAI_MODELS = [
  ...
  { provider: 'Groq', model: 'llama-3.1-8b-instant', kind: 'openai-compatible', base: 'https://api.groq.com/openai/v1' },
...
async function openaiCompatible(candidate, messages, maxTokens) {
  ...
  const r = await fetch(`${base}/chat/completions`, {
    ...
    body: JSON.stringify({ model: candidate.model, messages, ...(candidate.provider === 'OpenAI' ? { max_completion_tokens: maxTokens } : { max_tokens: maxTokens }), temperature: 0.2 })
  });
