const DEFAULT_ORDER = ['openai', 'google', 'groq', 'cerebras', 'mistral', 'openrouter', 'cloudflare'];
const MODELS = {
  openai: 'gpt-4o-mini',
...
function textFromMistral(j) { return j?.choices?.[0]?.message?.content || ''; }
function textFromOpenRouter(j) { return j?.choices?.[0]?.message?.content || ''; }
function textFromCloudflare(j) { return j?.result?.response || j?.result?.text || ''; }

function keyFor(provider) {
  return ({ google: 'GOOGLE_API_KEY', cloudflare: 'CLOUDFLARE_API_TOKEN', openrouter: 'OPENROUTER_API_KEY' }[provider]) || `${provider.toUpperCase()}_API_KEY`;
}
