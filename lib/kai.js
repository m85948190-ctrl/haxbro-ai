...
const KAI_MODELS = [
  ...
  { provider: 'Groq', model: 'openai/gpt-oss-20b', kind: 'openai-compatible', base: 'https://api.groq.com/openai/v1' },
  { provider: 'Mistral', model: 'mistral-small-latest', kind: 'openai-compatible', base: 'https://api.mistral.ai/v1' },
  { provider: 'OpenRouter', model: 'openrouter/auto', kind: 'openai-compatible', base: 'https://openrouter.ai/api/v1' },
  { provider: 'Hugging Face', model: 'Qwen/Qwen2.5-7B-Instruct', kind: 'openai-compatible', base: 'https://router.huggingface.co/v1' }
];
...
async function openaiCompatible(candidate, messages, maxTokens, imageData = '') {
  ...
  const payloadMessages = imageData && (candidate.provider === 'OpenAI' || candidate.provider === 'Groq')
...
export async function complete({ system = '', prompt, maxTokens = 1800, username = '', history = '', imageData = '' } = {}) {
  ...
  // Images require a vision-capable model. Use OpenAI first, then Gemini; do not
  // send image payloads to text-only fallbacks that cannot inspect the attachment.
  const candidates = imageData
    ? [KAI_MODELS.find(x => x.provider === 'OpenAI'), { ...KAI_MODELS.find(x => x.provider === 'Groq'), model: 'meta-llama/llama-4-scout-17b-16e-instruct' }].filter(Boolean)
    : KAI_MODELS;
  ...
  try {
    const result = await googleGenerate(messages, maxTokens, imageData); googleAttempt.status = 'success'; googleAttempt.elapsedMs = Date.now() - gt; attempts.push(googleAttempt);
