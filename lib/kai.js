const CORE_ORDER = ['groq', 'mistral', 'openrouter', 'huggingface', 'google', 'openai'];
const EXTRA_SLOTS = Array.from({length:8}, (_,i)=>`compat${i+1}`);
function configuredOrder(){ return [...CORE_ORDER, ...EXTRA_SLOTS].filter(p=>p.startsWith('compat') ? !!process.env[`${p.toUpperCase()}_API_KEY`] && !!process.env[`${p.toUpperCase()}_BASE_URL`] && !!process.env[`${p.toUpperCase()}_MODEL`] : true); }
const DEFAULT_ORDER = configuredOrder();
...
function isCompat(provider){return provider.startsWith('compat');}
function compatEnv(provider,suffix){return process.env[`${provider.toUpperCase()}_${suffix}`];}
function keyFor(provider) {
  if(isCompat(provider)) return `${provider.toUpperCase()}_API_KEY`;
...
async function callProvider(provider, { system, prompt, maxTokens, healthOnly }) {
  ...
  const model = isCompat(provider) ? compatEnv(provider,'MODEL') : MODELS[provider];
  const input = healthOnly ? 'Reply with OK.' : prompt;
  let url, headers, body, parse;
  if (isCompat(provider)) {
    url = `${String(compatEnv(provider,'BASE_URL')).replace(/\/+$/,'')}/chat/completions`;
  ...
  const response = await fetch(url, { method: 'POST', headers, body: JSON.stringify(body), signal: timeout(12000) });
...
export async function monitor({ order = DEFAULT_ORDER, system = 'You are a connectivity probe. Reply OK.' } = {}) {
  ...
  for (const provider of order) {
    ...
    try {
      ...
      results.push({ provider, model: isCompat(provider) ? compatEnv(provider,'MODEL') : MODELS[provider], status: missing ? 'not_configured' : 'offline', responseMs: err.elapsedMs ?? (Date.now() - started), error: err.details || err.message });
    }
  }
